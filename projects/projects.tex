...
\begin{table}[H]
    ...
    SD\_Log   & 4 Hz               & $\sim$175 ms      & Low      \\ \hline
    \end{tabular}
    \caption{\gls{rtos} tasks description}
    \label{table:tasks_details}
\end{table}

Beacause \texttt{IMU Step}'s priority is higher than \texttt{SD Log}'s priority and I want it to run at 100 Hz, I decided to use a semaphore on timer interuption to release the \texttt{IMU Step} task.
\begin{figure}[H]
    \centering
